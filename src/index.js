...
const BMAP_DRAWING_CIRCLE = 'circle'; // 鼠标画圆模式
const BMAP_DRAWING_RECTANGLE = 'rectangle'; // 鼠标画矩形模式
const BMAP_DRAWING_POLYGON = 'polygon'; // 鼠标画多边形模式

/**
 * 声明baidu包
 */
const _baidu = baidu || { guid: '$BAIDU$' };
const context = {};
 ...
 * @returns {Object} 目标对象
 */
_baidu.extend = function (target, source) {
 ...
 */
_baidu.lang.guid = function () {
	return `TANGRAM__${(context[_baidu.guid]._counter++).toString(36)}`;
};

context[_baidu.guid]._counter = context[_baidu.guid]._counter || 1;

/**
 * 所有类的实例的容器
 * key为每个实例的guid
 */
context[_baidu.guid]._instances = context[_baidu.guid]._instances || {};
 ...
 */
_baidu.lang.Class = function (guid) {
	this.guid = guid || _baidu.lang.guid();
	context[_baidu.guid]._instances[this.guid] = this;
};
 ...
 */
_baidu.lang.isString = function (source) {
 ...
 * @returns {boolean} 类型判断结果
 */
_baidu.lang.isFunction = function (source) {
 ...
 */
_baidu.lang.Class.prototype.toString = function () {
 ...
 * @grammar obj.dispose()
 */
_baidu.lang.Class.prototype.dispose = function () {
	delete context[_baidu.guid]._instances[this.guid];
	for (const property in this) {
		if (!_baidu.lang.isFunction(this[property])) {
 ...
 */
_baidu.lang.Event = function (type, target) {
	this.type = type;
 ...
 */
_baidu.lang.Class.prototype.addEventListener = function (type, handler, key) {
	if (!_baidu.lang.isFunction(handler)) {
		return;
	}
	if (!this.__listeners) {
		this.__listeners = {};
 ...
 */
_baidu.lang.Class.prototype.removeEventListener = function (type, handler) {
	if (_baidu.lang.isFunction(handler)) {
		handler = handler.hashCode;
	} else if (!_baidu.lang.isString(handler)) {
		return;
	}
	if (!this.__listeners) {
		this.__listeners = {};
 ...
 */
_baidu.lang.Class.prototype.dispatchEvent = function (event, options) {
	if (_baidu.lang.isString(event)) {
		event = new _baidu.lang.Event(event);
	}
	if (!this.__listeners) {
		this.__listeners = {};
	}
	...
	if (_baidu.lang.isFunction(this[p])) {
		// eslint-disable-next-line prefer-rest-params,prefer-spread
		this[p].apply(this, arguments);
	}
	if (typeof t[p] === 'object') {
		for (const i in t[p]) {
			if ({}.hasOwnProperty.call(t[p], i)) {
 ...
 * @see baidu.lang.Class
 */
_baidu.lang.inherits = function (subClass, superClass, className) {
	let key;
	...
	for (key in selfProps) {
		if ({}.hasOwnProperty.call(selfProps, key)) {
 ...
 */
_baidu._g = function (id) {
	if (_baidu.lang.isString(id)) {
		return document.getElementById(id);
	}
	return id;
};
_baidu.dom._g = _baidu._g;
 ...
 */
_baidu.insertHTML = function (element, position, html) {
	element = _baidu.dom.g(element);
	let range;
	let begin;
...
_baidu.dom.insertHTML = _baidu.insertHTML;
 ...
 * @returns {HTMLElement} 目标元素
 */
_baidu.ac = function (element, className) {
	element = _baidu.dom.g(element);
 ...
 * @meta standard
 */
_baidu.event._listeners = _baidu.event._listeners || [];
 ...
 */
_baidu.on = function (element, type, listener) {
	type = type.replace(/^on/i, '');
	element = _baidu._g(element);
	let realListener = function (ev) {
		// 1. 这里不支持EventArgument,  原因是跨frame的事件挂载
		// 2. element是为了修正this
		listener.call(element, ev);
	};
	const lis = _baidu.event._listeners;
	const filter = _baidu.event._eventFilter;
	let afterFilter;
	let realType = type;
 ...
 * @returns {HTMLElement|window} 目标元素
 */
_baidu.un = function (element, type, listener) {
	element = _baidu._g(element);
 ...
 * @return {Event}
 */
_baidu.getEvent = function (event) {
	return window.event || event;
};
_baidu.event.getEvent = _baidu.getEvent;
 ...
 * @return {Target}
 */
_baidu.getTarget = function (event) {
	const _event = _baidu.getEvent(event);
	return _event.target || _event.srcElement;
};
_baidu.event.getTarget = _baidu.getTarget;
 ...
 * @meta standard
 */
_baidu.preventDefault = function (event) {
	const _event = _baidu.getEvent(event);
...
_baidu.event.preventDefault = _baidu.preventDefault;

/**
 * 停止事件冒泡传播
 * @param {Event}
 */
_baidu.stopBubble = function (event) {
	const _event = _baidu.getEvent(event);
...
_baidu.event.stopBubble = _baidu.stopBubble;

_baidu.browser = _baidu.browser || {};
...
if (/msie (\d+\.\d)/i.test(navigator.userAgent)) {
	 ...
	 */
	_baidu.browser.ie = document.documentMode || +RegExp.$1;
	_baidu.ie = _baidu.browser.ie;
}
...
function Mask() {
	...
	for (const pro in Mask.prototype) {
		if ({}.hasOwnProperty.call(Mask.prototype, pro)) {
...
function initMask() {
	 ...
	 */
	Mask.prototype.dispatchEvent = _baidu.lang.Class.prototype.dispatchEvent;
	Mask.prototype.addEventListener =		_baidu.lang.Class.prototype.addEventListener;
	Mask.prototype.removeEventListener =		_baidu.lang.Class.prototype.removeEventListener;
	...
	Mask.prototype._bind = function () {
		...
		while (index--) {
			_baidu.on(container, events[index], domEvent);
		}

		// 鼠标移动过程中，到地图边缘后自动平移地图
		_baidu.on(container, 'mousemove', (e) => {
	...
	Mask.prototype.getDrawPoint = function (e) {
		const map = this._map;
		let trigger = _baidu.getTarget(e);
...
function DrawingTool(drawingManager, drawingToolOptions) {
	...
	for (const pro in DrawingTool.prototype) {
		if ({}.hasOwnProperty.call(DrawingTool.prototype, pro)) {
...
function initDrawingTool() {
	...
	DrawingTool.prototype._bind = function () {
		const me = this;
		_baidu.on(this.panel, 'click', (e) => {
			const target = _baidu.getTarget(e);
	...
	DrawingTool.prototype.setStyleByDrawingMode = function (drawingType) {
		...
		for (let i = 0, len = boxs.length; i < len; i += 1) {
			const box = boxs[i];
...
_baidu.lang.inherits(DrawingManager, _baidu.lang.Class, 'DrawingManager');
...
DrawingManager.prototype._bindCircle = function () {
	...
	const startAction = function (e) {
		...
		_baidu.on(document, 'mouseup', endAction);
	};
...
DrawingManager.prototype._bindPolylineOrPolygon = function () {
	 ...
	 * 鼠标移动过程的事件
	 */
	const mousemoveAction = function (e) {
	 ...
	 * 鼠标双击的事件
	 */
	const dblclickAction = function (e) {
		_baidu.stopBubble(e);
		isBinded = false;
		mask.disableEdgeMove();
		...
		const calculate = me._calculate(overlay, points.pop());
...
DrawingManager.prototype._bindRectangle = function () {
	...
	const endAction = function () {
		const calculate = me._calculate(polygon, polygon.getPath()[2]);
...
DrawingManager.prototype._calculate = function (overlay, point) {
	...
	if (this._enableCalculate && BMapLib.GeoUtils) {
		...
		case '[object Polyline]':
			result.data = _BMapLib.GeoUtils.getPolylineDistance(overlay);
			break;
		case '[object Polygon]':
			result.data = _BMapLib.GeoUtils.getPolygonArea(overlay);
			break;
		case '[object Circle]':
